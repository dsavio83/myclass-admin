                    ...
                    try {
                        ...
                        const saveResponse = await fetch('/api/content/cloudinary-save', {
                            ...
                            body: JSON.stringify({
                                ...
                                size: result.bytes,
                                mimeType: file.type,
                                resourceType: 'video' // Cloudinary treats audio as video resource type often
                            })
                        });

                        if (saveResponse.ok) {
                            showToast('Audio uploaded and saved successfully!', 'success');
                            setIsUploading(false);
                            onAdd();
                        } else {
                            const errorData = await saveResponse.json();
                            throw new Error(errorData.message || 'Failed to save audio metadata');
                        }
