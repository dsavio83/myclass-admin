...
import { TrashIcon, UploadCloudIcon, PlusIcon, DownloadIcon, XIcon, LinkIcon, CheckCircleIcon } from '../icons/AdminIcons';
...
const BeautifulQuestionCard: React.FC<{
    ...
    const colorSchemes = [
        { bg: 'from-orange-400 to-rose-600', icon: 'bg-white/20' },
        { bg: 'from-indigo-400 to-blue-600', icon: 'bg-white/20' },
        { bg: 'from-amber-400 to-orange-600', icon: 'bg-white/20' },
        { bg: 'from-teal-400 to-emerald-600', icon: 'bg-white/20' },
        { bg: 'from-fuchsia-400 to-purple-600', icon: 'bg-white/20' },
            ...
            <div className="p-5 flex flex-col flex-1">
                ...
                <div className="mt-auto flex items-center justify-between">
                    <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded-lg text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                        {content.metadata?.examCategory || 'Exam Paper'}
                    </div>
                    <button
                        onClick={(e) => { e.stopPropagation(); onDownloadClick(); }}
                        disabled={downloading}
                        className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 transition-all shadow-md shadow-indigo-500/20 disabled:opacity-50"
                    >
...
const UploadForm: React.FC<{ lessonId: string; onUploadSuccess: () => void; onCancel: () => void; category?: string; }> = ({ lessonId, onUploadSuccess, onCancel, category }) => {
        ...
        const fetchDefaults = async () => {
            try {
                ...
                if (hierarchy) {
                    ...
                    setFolderPath(`${path}/QuestionPapers`);
                }
    ...
    const categories = ['Monthly Test', 'Summative Exam', 'Model Exam', 'SSLC Exam', 'Revision Test', 'District Paper'];
    ...
    const handleLinkSave = async () => {
        ...
        try {
            await api.addContent({
                title, body: url, lessonId, type: 'questionPaper', category: category || 'standard',
            ...
            <div className="w-full md:w-5/12 p-6 sm:p-8 bg-gray-50/50 dark:bg-gray-900/30 border-r border-gray-100 dark:border-gray-700 flex flex-col">
                <div className="mb-8">
                    <div className="w-12 h-12 bg-indigo-100 dark:bg-indigo-900/30 rounded-xl flex items-center justify-center mb-4">
                        <QuestionPaperIcon className="w-6 h-6 text-indigo-600 dark:text-indigo-400" />
                    </div>
                    <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-2">Configure Paper</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Set the exam details and choose your storage strategy.</p>
                </div>
                ...
                <div className="space-y-6 flex-1">
                    <div className="grid grid-cols-1 gap-4">
                        <div>
                            ...
                            <select
                                ...
                                className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                            >
                                {categories.map(c => <option key={c} value={c}>{c}</option>)}
                            </select>
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Exam Name / Year</label>
                            <input
                                type="text"
                                value={examDetail}
                    ...
                    <div className="p-4 bg-indigo-50/50 dark:bg-indigo-900/20 rounded-xl border border-indigo-100/50 dark:border-indigo-800/50">
                        <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider mb-1">Generated Title</p>
                        <p className="text-sm font-bold text-gray-800 dark:text-white">{examCategory} {examDetail || '...'}</p>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-2 p-1 bg-gray-200/50 dark:bg-gray-700/50 rounded-xl">
                            <button
                                ...
                                className={`py-2.5 text-xs font-bold rounded-lg transition-all ${activeTab === 'upload' ? 'bg-white dark:bg-gray-600 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-gray-500'}`}
                            >
                                <UploadCloudIcon className="w-4 h-4 mx-auto mb-1" />
                                UPLOAD
                            </button>
                            <button
                                onClick={() => setActiveTab('link')}
                                className={`py-2.5 text-xs font-bold rounded-lg transition-all ${activeTab === 'link' ? 'bg-white dark:bg-gray-600 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-gray-500'}`}
                            >
                ...
                <div className="mt-8 pt-6 border-t border-gray-100 dark:border-gray-700">
                    <p className="text-xs text-gray-500 italic">Target folder: <span className="font-mono text-indigo-600 dark:text-indigo-400">{folderPath}</span></p>
                </div>
            </div>
                ...
                <div className="w-full max-w-sm">
                    {activeTab === 'upload' ? (
                        <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                            <div className={`relative group cursor-pointer transition-all duration-300 ${file ? 'bg-indigo-50/50 dark:bg-indigo-900/20 border-indigo-400' : 'hover:bg-gray-50 dark:hover:bg-gray-700/50 border-gray-300'} border-2 border-dashed rounded-3xl p-10 text-center flex flex-col items-center justify-center`}>
                                <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" onChange={handleFileChange} accept=".pdf" />
                                <div className={`p-5 rounded-2xl mb-4 transition-transform group-hover:scale-110 ${file ? 'bg-indigo-500 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-400'}`}>
                                ...
                                {file ? (
                                    <>
                                        ...
                                        <p className="text-xs text-indigo-500 font-medium">Click to change file</p>
                                    </>
                                ) : (
                                    <>
                                        <p className="text-lg font-bold text-gray-700 dark:text-gray-200">Drop Paper PDF here</p>
                                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">or click to browse</p>
                                    </>
                                )}
                            </div>
                            <button
                                disabled={!file || !examDetail}
                                onClick={handleUpload}
                                className="w-full py-4 px-6 bg-gradient-to-br from-indigo-600 to-blue-700 text-white rounded-2xl font-bold shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/50 hover:scale-[1.02] transition-all disabled:opacity-50 flex items-center justify-center gap-3"
                            >
                        ...
                        <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                                />
                                <p className="text-xs text-gray-500 mt-2">Paste the direct link to a hosted PDF file.</p>
                            </div>
                            <button
                                disabled={!url || !examDetail || isSaving}
...
export const QuestionPaperView: React.FC<QuestionPaperViewProps> = ({ lessonId, user, category }) => {
    const [version, setVersion] = useState(0);
    const { triggerContentUpdate, updateVersion } = useContentUpdate();
    const [showUploadForm, setShowUploadForm] = useState(false);
    const { data: grouped, isLoading } = useApi(() => api.getContentsByLessonId(lessonId, ['questionPaper'], (user.role !== 'admin' && !user.canEdit), category), [lessonId, version, user, updateVersion, category]);
    ...
    const handleTogglePublish = async (item: Content) => {
        try {
            ...
            setVersion(v => v + 1);
            triggerContentUpdate();
            showToast(`Paper ${newStatus ? 'published' : 'unpublished'}`, 'success');
        } catch (error) {
    ...
    const handleView = async (url: string, id: string) => {
        ...
        trackView(lessonId, 'questionPaper', id).catch(() => { });
    };
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden bg-gray-50/30 dark:bg-gray-900/30">
            {canEdit && papers.length > 0 && (
                <ContentStatusBanner
                    publishedCount={papers.filter(a => a.isPublished).length}
                    unpublishedCount={papers.filter(a => !a.isPublished).length}
                />
            )}
            ...
            <div className="p-4 sm:p-6 lg:p-8 flex-1 overflow-hidden flex flex-col">
                <div className="flex justify-between items-center mb-6 shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg">
                            <QuestionPaperIcon className="w-6 h-6 sm:w-8 sm:h-8 text-indigo-600 dark:text-indigo-400" />
                        </div>
                        <h1 className="text-xl sm:text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-gray-900 to-indigo-600 dark:from-white dark:to-indigo-400">Question Papers</h1>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto min-h-0 custom-scrollbar pr-2">
                    {isLoading && (
                        <div className="flex flex-col items-center justify-center py-20">
                            ...
                            <p className="text-gray-500 font-medium">Gathering question papers...</p>
                        </div>
                    )}
                    ...
                    {!isLoading && !showUploadForm && papers.length > 0 && (
                    ...
                    {!isLoading && !showUploadForm && papers.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-32 bg-white dark:bg-gray-800/30 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-700/50">
                            <div className="p-6 bg-gray-50 dark:bg-gray-800 rounded-full mb-6">
                                <QuestionPaperIcon className="w-16 h-16 text-gray-300 dark:text-gray-600" />
                            </div>
                            <h3 className="text-xl font-bold text-gray-700 dark:text-gray-300 mb-2">No papers yet</h3>
                            <p className="text-gray-500 dark:text-gray-400 max-w-sm text-center">
                                {canEdit ? "Ready to add some assessments? Click 'Add New' to upload your first question paper." : "Check back later for question papers related to this lesson."}
                            </p>
                            {canEdit && (
                                <button onClick={() => setShowUploadForm(true)} className="mt-8 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-bold transition-all shadow-lg shadow-indigo-500/30">
                                    Upload First Paper
                                </button>
                            )}
                        </div>
                ...
                <ConfirmModal isOpen={confirmModal.isOpen} onClose={() => setConfirmModal({ isOpen: false, onConfirm: null })} onConfirm={confirmModal.onConfirm} title="Remove Paper" message="Are you sure you want to remove this paper?" />
